    ...
    <div class="br-mainpanel">
      ...
      <div class="br-pagebody" >
            ...
            <div class="col-xl-6">
			 ...
			 <center><h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10">Upload Your Photo</h6></center>
                 
				 <?php 
              if($row['photo']==NULL){
                
                ?>  
              <center>  <img src="img/profile.webp" style="border-radius:10px; width:100px;"/></center>
                <?php
              }else{
                  
                  ?>
                 <center> <img src="kyc/<?php echo $row['photo'];?>" style="border-radius:10px; width:100px;"/></center>
                  <?php
              }
              ?>
						...
						<?php
                           if(isset($_POST['submit'])){
                            ...
                            $extensions=array('image/jpg','image/jpeg','image/png');
                               	    $query=mysqli_query($con,"UPDATE `admin` SET `photo`='$image' WHERE `userid`='$userid'");
                               	    $newname = $image;  
                                      $target = 'kyc/'.$newname;
                                      move_uploaded_file( $_FILES['image']['tmp_name'], $target);
                               	     if($query){
                               	          echo '<script>alert("UPDATE SUCCESSFUL.");window.location.assign("update_photo.php");</script>';
                               	     }else{
                               	         echo '<script>alert("Sumthing Went Wrong.");window.location.assign("update_photo.php");</script>'; 
                               	     }
